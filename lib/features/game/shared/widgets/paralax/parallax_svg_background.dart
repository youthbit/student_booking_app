...
class ParallaxBackgroundSettings {
  ...
  int get hashCode => quiver.hash3(
        deepEffectMatrix.hashCode,
        shadowColor.hashCode,
        shadowSigma.hashCode,
      );
}
...
class _ParallaxBackgroundPainter extends CustomPainter {
  ...
  Future<void> paint(Canvas canvas, Size size) async {
    ...
    var scaleXFactor = 1.0;
    var scaleYFactor = 1.0;
  ...
  void _drawPath(Canvas canvas, DrawableRoot drawable) {
    final deepEffectMatrix =
        settings?.deepEffectMatrix ?? Matrix4.identity().storage;

    // Draw shadow
    if (disableShadow != true) {
      final shadowDrawable = drawable.mergeStyle(
        DrawableStyle(
          fill: DrawablePaint(
            PaintingStyle.fill,
    ...
    if (disableDeepEffect != true) {
      final deepDrawable = drawable.mergeStyle(
        const DrawableStyle(
          fill: DrawablePaint(
            PaintingStyle.fill,
